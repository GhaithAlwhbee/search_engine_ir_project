...
class MapEvaluation:
    ...
    async def evaluate(self,dataSetIndex):
        ...
        for i,qrel in enumerate(self.listOfqrel):
            # print(qrel)
            self.mapAtk.append(await self.evaluate_query(qrel,dataSetIndex))
            if (i+1) % 50 == 0:
                print(f'Processed {i+1} qrel')
        for x in self.mapAtk:
            sum += x

        return sum/len(self.mapAtk)
    ...
    #     numOfReleventDocs = self.NOR(pAtk)
    #     if(numOfReleventDocs == 0): return sum
    #     return sum/numOfReleventDocs
    ...
    async def reCall(self,queryObject: QrelQuery,dataSetIndex: int):
        retrived_docs = []
        ...
        for i,doc_id in enumerate(retrived_docs.json()):
            ...
            # if i >9:break
        # print("pAtk:",pAtk)
        return numOfRelevantDocs/len(queryObject.answer_pids)
